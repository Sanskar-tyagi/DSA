...
public class NoramlQueue {
    ...
    public NoramlQueue(int cap) {
        data = new int[cap];
        size = 0;
        first = 0;// We create attributes of the class
    }

    static int Size(NoramlQueue Q) { // Size To get the current size of Deque
        int n = Q.size;
        return n;
    }

    static void addLast(NoramlQueue Q, int a) {// ? As the Deque follows FIFO we fist check if the size of the current
                                               // ? Queue and the Capacity avilable
        int n = Q.size;
        int max = Q.data.length;
        if (n < max && n != 0) { // Then we check if its less than n and its not the first Variable
            Q.data[n] = a;
            Q.size += 1;
    ...
    static void RemoveFirst(NoramlQueue Q) {
        int n = Q.size;
    ...
    static int Peek(NoramlQueue nq) {
        return nq.first;
    }

    static void AddValAt(NoramlQueue Q, int val, int k) {
        int n = Q.size;
        int max = Q.data.length;
        if (k < max && n != 0) { // Then we check if its less than n and its not the first Variable
            if (k == 0) {
                Q.first = val;
                int temp = Q.data[k];
                Q.data[k] = val;
    ...
    static void display(NoramlQueue ns) {
        for (int i : ns.data) {
            if (i != 0 && i + 1 != 0) {
    ...
    public static void main(String[] args) {
        ...
        int n = sc.nextInt();
        NoramlQueue ns = new NoramlQueue(n);// Creating the Instance of the class
        ...
        System.out.println("Enter Value TO add K");
        x = sc.nextInt();
        ...
        System.out.println("Queue Peek " + Peek(ns));// In FIFO
        display(ns);
        System.out.println("Queue After Peek ");// In FIFO
        RemoveFirst(ns);
        System.out.println("Queue After POP ");// In FIFO
        display(ns);
        RemoveFirst(ns);
